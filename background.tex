...
Cloud computing is a paradigm that allows Cloud users and End users to acess Cloud services and applications based on their requirements regardless of where the services are. The advent of Cloud computing matches the trend of fast growing applications where these applications are deploying in a third-party data centers and application users can use them without installing on their local computers.
...
Cloud computing involves three stakeholders {Jennings:2015ht} (see Figure \ref{fig:stakeholders}: Cloud provider, Cloud user and End User. Each of them has their responsibility, goal, and objectives. 
\begin{itemize}
...
In this thesis, we focus on a core issue of helping Cloud providers to increase their profits by optimizing the resource allocation in data centers. 
