...
Mann's research \cite{Mann:2016hx} is the earliest study that realized the VM selection and VM placement should be considered together. Although his paper does not mention the term ``container placement'', it actually allocates containers to VMs. They apply a fixed VM placement algorithm and considering a series of VM selection algorithms such as simple selection \cite{Ganesan:2012eb},  Multiple selection, Maxsize, Consolidation-friendly. They show that the results improved with an algorithm which has more knowledge of the PMs' capacity. However, he still consider these two operations separately, while in our thesis, we are going to consider it together.
