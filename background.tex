...
GA \cite{Holland:1962fy} was introduced by Holland. Since then, a large number of applications have applied GA \cite{DeJong:1992vk, DeJong:1992ws} as their search mechanism.
...
\begin{enumerate}
	\item \emph{Application initial placement} is applied when new applications or new VMs arrive and the problem is to allocate them into a minimum number of PMs.
...
As Section \ref{scenarios} mentioned, application initial placement is one of the major process in resource management.
Cloud users request a number of resources represented by VMs or containers with certain configurations, the task for cloud provider is to deploy these resources (VMs or containers) to a minimum number of physical machines (PM) because the energy consumption is proportional to the active PMs.

This section first discusses container-based approaches and their drawbacks. Then, based on their drawbacks, we will review the reasons for our first objective (see \ref{sec:obj1}): propose EC-based approaches for solving the joint allocation of containers and VMs. Since there are only few research study the container-based placement and none of them considers it as a bilevel optimization problem, we will have to propose a model to formulate this problem. Thus, we will review a number of VM-based placement and mainly study the following five aspects: resource model, power model, wastage model, objective, and algorithm.
% In energy-aware resource management, the initialization has the objective of minimizing the used PMs. In literature, the static initialization problem is often modeled as the vector bin packing problem. Each application represents an item and PMs represents bins.

\subsubsection{Container-based Application Initial Placement}
\label{container-based-placement}
Piraghaj et al \cite{Piraghaj:2016bw} first propose an architecture for container-based resource management. In this architecture, it allows the cloud providers to customize the size or type of VM when allocating applications instead of traditional fixed size VM. They determine the size of VM by two steps, in a pre-execution phase, they perform clustering technique on historical workload data from Google Cluster Data. In this way, they assert that the applications with similar workload pattern are clustered into the same group. In the execution phase, when a number of deployment requests of container arrives, they perform First Fit on existing VMs. If new VMs need to be initiated, they first determines the size of VMs by some policies such as utilization based Resource allocation or requested resource allocation. Then, the containers are allocated to certain size of VMs. 
Then, they again apply First Fit to allocate VMs to PMs. 

Mann's research \cite{Mann:2016hx} is the earliest study which realizes when deploying applications or containers, the VM size selection and VM placement should be considered together because they are interact with each other. The paper claims ``it would be advantageous to combine into a single VM as many tasks as possible, on the other hand, too big VMs limit the consolidation possibilities''.  They apply a fixed VM placement algorithm and considering a series of VM selection algorithms such as simple selection \cite{Ganesan:2012eb},  Multiple selection, Maxsize, Consolidation-friendly. The results show that the solutions improved with the VM selection has more knowledge of the PMs' capacity.

There are mainly three reasons for us to propose a distinct approach from Piraghaj \cite{Piraghaj:2016bw} to solve the container-based placement problem. First, their proposed architecture assumes arbitrary size of VM can be created when requests arrive. In order to adapt to these architecture, cloud providers need to pay huge amount of upfront investment to modify the current system. While our assumption is that the container-based architecture is based on traditional IaaS, where fix-size VMs provide the fundamental resources. Second, from the perspective of energy efficiency, the allocation of container and VM interact with each other. That is, the minimum number of VMs does not necessary lead to the minimum number of PMs, because the type of VMs also affect the results. This inspires us to simultaneously allocate containers and VMs. Third, their approach does not consider the OS requirement of containers. We argue that this is another critical reason for deploying containers into different VMs besides the ``limit the consolidation possibilities'' argument states by Mann.
...
This section reviews several research focused on VM-based placement, as mentioned, we mainly focus on their models.
