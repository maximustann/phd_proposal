...
In this section, we summarize the related works in the terms of three resource management scenarios: application initial placement, periodic consolidation and dynamic consolidation, corresponding to our proposed objectives. 
We would first analyze the container-based approaches, discuss their models and problem solving methodologies in each scenario. So far, there are few research about container-based consolidation. In order to borrow some useful ideas from previous studies, we will discuss some similar VM-based approaches. 
...
Cloud users request a number of resources represented by VMs or containers with certain configurations, the task for cloud provider is to deploy these resources (wrapped with VMs or containers) to a minimum number of physical machines (PM) as the number of PM is proportional to the energy consumption. The optimization objective is to minimize the energy consumption.

This section first discusses container-based approaches including modeling and methodology. Then, we will discuss the problem in their approaches and review the reasons that their approach cannot obtain the near optimal energy consumption. Furthermore, in our first objective (Section \ref{sec:obj1}), we intend to propose a bilevel optimization to solve the problem. So far there is no research consider the placement problem as a bilevel optimization problem. Therefore, we will review a number of VM-based placement to learn from their models and problem solving approaches. 
