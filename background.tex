...
\bx{To give an example of how Cloud computing works (see Figure \ref{fig:stakeholders}), consider the case:} a \emph{Cloud provider} builds a data center contained thousands of servers. These servers connect with network. 
...
\bx{The major expense of a cloud provider is energy consumption \cite{Kaplan:up01fR-k} and physical machines (PMs) (e.g servers) contribute a majority of the energy.} As shown in Figure \ref{fig:consumption}, both cooling system and physical machines (PMs) account for 40\% of energy. However, PMs' energy efficiency are low on average  \cite{Hameed:2016cmb}.
The reason for low energy efficiency is the disproportionate between the utilization of PMs and the energy consumption of PMs. \howto{For example, when CPU utilization of a PM is 15\%, the energy consumption of the PM is 70\% of its peak time.} Therefore, 
...
This section will first introduce and compare two types of virtualization: container-based and VM-based and then illustrate 
...
\bx{Resource management uses virtualization technologies \cite{Uhlig:2005do} to achieve a finer granularity management than a traditional way.} In comparison with traditional management -- allocating an application to a PM -- virtualized management partitions PM's resources (e.g. CPU, memory and disk) into several independent units and allocates applications into these units. 
...
\bx{A VM-based virtualization has a three-layers of structure: PM-Hypervisor-VM (see Figure \ref{fig:comparison} left-hand side).} 
...
\bx{In comparison with OS container, application container is much more flexible because each container to have its separated environment (e.g. libraries) for applications.} Furthermore, application containers provide a finer granularity of resource management by enabling an application level of operations including deployment, scaling, and migration.  
...
\bx{This section summarizes three disadvantages of VM-based virtualization in terms of energy efficiency and shows container-based can overcome these disadvantages based several research~\cite{Felter:2015ki, Xavier:2013fy, Dua:2014bw}.}  
...
\bx{In summary, container-based virtualization has the potential to improve further improve the energy efficiency than VM-based virtualization.} No matter cloud uses which virtualization technology, cloud often deals with three placement decision scenarios. 
...
\paragraph{Initial placement of application} is applied when new applications arrived. The task is to place applications into a set of PMs \cite{Mishra:2012kx} so that PMs satisfy all applications' resource demands and minimize energy consumption.
...
This section first describes a commonly used energy model for VM-based virtualization. Then, we review a number of traditional approaches for VM-based initial placement . We mainly study the following five aspects: resources, power model, wastage model, objective, and algorithm.
...
\paragraph{Energy model in VM-based cloud: Vector Bin Packing Model}
...
% Based on our knowledge, periodic placement has not been studied in the new context of container-based Cloud. 
% Therefore, we are motivated to explore the migration models and algorithms.
