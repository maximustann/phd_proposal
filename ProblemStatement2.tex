...
\begin{itemize}
	\item IaaS, Cloud provider offers the fundamental resources such as CPU cores, RAMs, and network bandwidth. These resources are encapsulated in a set of fixed size of VMs. The 'size' means a VM contains a certain number of resources such as 2-cores, 1 GB RAM, and they are predefined by the Cloud provider.
% Cloud providers offer two price model: reservation plus an auto-scaling price model to Cloud users. Reservation means Cloud users initially need to reserve a number of resources (e.g a type of VM). Auto-sca

 	In order to deploy their applications, Cloud users have to estimate how much resource do they need. Then, they can choose one or more VMs from a few fixed types. After receiving the requests, Cloud providers choose a set of PMs which contains sufficient resources, provision the required VMs in them. Then, the VMs are assigned to the Cloud users. Cloud providers manage the VMs by monitoring the resource utilization of VMs and PMs. If a VM has run out of resources, an auto-scaling strategy is used to provision new VMs to Cloud users. Finally, Cloud providers optimize the locations of VMs so that the energy consumption of data center is minimized.
...
In this proposal, we focus on an important issue of helping Cloud providers to improve their profit. Our approach is to reduce the expense of energy consumption in data centers by providing a series of optimization methods in Cloud resource management.
