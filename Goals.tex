...
\begin{enumerate}
	...
	\item How to improve the quality of solution in terms of effectiveness or efficiency?

	Although nested approaches have been reported effective, they are very time consuming. Therefore, this sub objective intends to explore other directions to improve either effectiveness or efficiency. One possible direction is single-level reduction, which reduces the bilevel problem into a single dimensional problem. Clustering approaches such as K-means, DBScan can be useful in categorizing containers in predefined groups. Then, complementary containers can be grouped and allocated. 
...
As previously (see Section  \ref{sec:motivation}) mentioned, the task is multi-objective since the number of VM migration has to be minimized while keep the overall energy low. In addition, periodic optimization is a time-dependent problem which means the optimal consolidation in previous operation might lead to more migrations in the current consolidation. The reason for the migration is the variation of workload. According to Bobroff \cite{Bobroff:2007ec}, the workload can be roughly grouped into three categories: static, periodical and variant (unpredictable). Therefore, in order to design a robust server consolidation algorithm, workload types must be taken into account. We also use three questions to guide our objective.
...
\begin{enumerate}
	\item How to design an EC-based approach which solve the multi-objective joint allocation problem considering static workload?

	In this sub-objective, we will mainly focus on developing EC-based approaches to solve the multi-objective joint allocation problem. We consider two objectives: minimizing the number of migration and minimizing the overall energy consumption. Since both container and VM can be migrated, that is, multi-objective exists on both levels. We will start from a simple case with considering multi-objective in lower level: Minimizing container migration and energy consumption.  Currently,  there are few studies using EC methods \cite{Yin:2000bt, Deb:2010in} for multiobjective bilevel optimization. We will investigate which one is more suitable for this binary problem. Furthermore, we will start from one dimensional of resource: CPU utilization. Since the focus is on how to adapt the problem with an EC approach, we only consider the static workload which assumes the workload does not change during time.  We can utilize the representation and problem models from previous objective. However, we need to propose new genetic operators to adapt the multi-objective problem.

	% like the case in single objective problem, we need to develop new representations, genetic operators.

	\item How to design an EC-based approach which considers periodic workloads?

	In this sub-objective, we will design a time-series aware EC-based server consolidation approach with considering periodic workload. Since periodical workload is one of the major type of workload, we will consider how to adapt it into the previously designed multi-objective algorithm. Previous research also consider periodic workload \cite{Meng:2010gh}, however, they only consider combining paired complementary workloads, therefore, it is very limited. In contrast, we are going to consider combining multiple periodic workloads. This task is more challenging since it is the number of combination is very large. We will start from the simplest case of  stationary workload (the mean value of the series that remains constant over a time period). Depending on the measurement, direct and indirect representation will be explored. Direct representation may represent the solution with a vector of segmented workload. While indirect representation may represent the solution with .
	...
	\item How to design an EC-based approach which considers unpredictable workloads?

	Unpredictable workloads is the major challenging in server consolidation, since their behaviors are unknown, it is difficult to come up with an optimal plan for co-placement them. Such workloads occur quite often in the real world. For instance, burst increases of on-line broadcasting website when breaking news is happening. The occurrence of peaks and duration cannot be foreseen. In order to 
