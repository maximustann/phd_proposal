...
In a container-based environment, it involves three steps . 
	...
	To solve a dynamic placement with large number of variables, dispatching rule can be an useful tool \cite{Sarin:2011fu}. In this scenario, a dispatching rule is considered as a function that determines the priorities of servers that a container can be placed.
	We intend to develop a hyper-heuristic method - genetic programming (GA) technique \cite{Banzhaf:1998wc} or artificial immune system \cite{Hofmeyr:2000ju}- to automatic evolve dispatching rules to solve this problem.
